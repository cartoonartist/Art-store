...
// Your Google Form URL (the one that ends with /formResponse)
const GOOGLE_FORM_URL = "https://docs.google.com/forms/d/e/1FAIpQLSfs2bPWuTaBA4Yjh-jyelwRG8mn09nIFdr0ybaDzJKE7k4ugA/formResponse";
...
function viewCart() {
    ...
    for(let i = 0; i < cart.length; i++) {
        ...
        modalHtml += `
            <div style="border:1px solid #ddd; margin:10px 0; padding:10px; border-radius:5px; display:flex; gap:15px; align-items:center;">
                ${productImage ? `<img src="${productImage}" style="width:60px; height:60px; object-fit:cover; border-radius:8px;" onerror="this.style.display='none'">` : ''}
                <div style="flex:1;">
                    <h4 style="margin:0 0 5px 0;">${item.product}</h4>
                    <p style="margin:0; color:#00d4ff;">Price: ₹${item.price}</p>
                </div>
                <button onclick="removeFromCart(${i})" style="background:#ff0080; color:white; border:none; padding:5px 10px; border-radius:5px; cursor:pointer;">Remove</button>
            </div>
        `;
    }
        ...
        modalHtml += `
            <div style="border:1px solid #ddd; margin:10px 0; padding:10px; border-radius:5px; display:flex; gap:15px; align-items:center;">
                ...
                <div style="flex:1;">
                    ...
                    <p style="margin:0; color:#00d4ff;">Price: ₹${productPrice}</p>
                </div>
                ...
                <button onclick="removeFromWishlist(${i})" style="background:#ff0080; color:white; border:none; padding:5px 10px; border-radius:5px; cursor:pointer;">Remove</button>
            </div>
        `;
    }
...
function removeFromWishlist(index) {
    ...
    updateCounters();
    closeWishlistModal();
    viewWishlist(); // Refresh wishlist view
}
...
// ===========================
// SUBMIT ORDER TO GOOGLE FORM (WORKING METHOD)
// ===========================
...
if(submitBtn) {
    ...
    submitBtn.addEventListener("click", async function() {
        ...
        // Build the Google Form URL with all parameters (this opens the form with pre-filled data)
        ...
        // Open the form in a new tab (customer just needs to upload images and submit)
