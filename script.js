...
function getProductImage(productName) {
    let product = allDrawings.find(d => d.name === productName);
    if(product) return product.image;
    product = allKeychains.find(k => k.name === productName);
    if(product) return product.image;
    return null;
}
...
function getProductPrice(productName) {
    ...
    if(product) {
        ...
        return discount > 0 ? Math.floor((product.price + product.delivery) * (1 - discount)) : product.price + product.delivery;
    }
    product = allKeychains.find(k => k.name === productName);
    if(product) {
        const discount = getTotalDiscount('keychain');
        return discount > 0 ? Math.floor(product.price * (1 - discount)) : product.price;
    }
    return 0;
}

// ===========================
// CART VIEW WITH IMAGES
// ===========================
function viewCart() {
    if(cart.length === 0) {
    ...
    for(let i = 0; i < cart.length; i++) {
        ...
        modalHtml += `
            <div style="border:1px solid #ddd; margin:10px 0; padding:10px; border-radius:8px; display:flex; gap:15px; align-items:center; background:#f9f9f9;">
                <div style="width:70px; height:70px; background:#eee; border-radius:8px; overflow:hidden;">
                    ${productImage ? `<img src="${productImage}" style="width:100%; height:100%; object-fit:cover;" onerror="this.src='images/placeholder.jpg'">` : '<div style="width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:30px;">🖼️</div>'}
                </div>
                <div style="flex:1;">
                    <h4 style="margin:0 0 5px 0;">${item.product}</h4>
                    <p style="margin:0; color:#00d4ff; font-weight:bold;">₹${item.price}</p>
                </div>
                <button onclick="removeFromCart(${i})" style="background:#ff0080; color:white; border:none; padding:8px 12px; border-radius:5px; cursor:pointer;">Remove</button>
            </div>
        `;
    }
    ...
    modalHtml += `
                ...
                <button onclick="checkoutCart()" style="width:100%; padding:12px; background:#00d4ff; border:none; border-radius:8px; font-weight:bold; cursor:pointer; margin-top:10px;">Proceed to Checkout</button>
            </div>
        </div>
    `;
    ...
    for(let i = 0; i < wishlist.length; i++) {
        ...
        modalHtml += `
            <div style="border:1px solid #ddd; margin:10px 0; padding:10px; border-radius:8px; display:flex; gap:15px; align-items:center; background:#f9f9f9;">
                ...
                <div style="flex:1;">
                    <h4 style="margin:0 0 5px 0;">${item}</h4>
                    <p style="margin:0; color:#00d4ff; font-weight:bold;">₹${productPrice}</p>
                </div>
                <button onclick="addToCartFromWishlist('${item.replace(/'/g, "\\'")}')" style="background:#00d4ff; border:none; padding:8px 12px; border-radius:5px; cursor:pointer; margin-right:5px;">Add to Cart</button>
                <button onclick="removeFromWishlist(${i})" style="background:#ff0080; color:white; border:none; padding:8px 12px; border-radius:5px; cursor:pointer;">Remove</button>
            </div>
        `;
    }
...
// ===========================
// SUBMIT ORDER - AUTO SUBMIT TO GOOGLE FORM (NO REDIRECT, NO NEW TAB)
// ===========================
...
if(submitBtn) {
    ...
    submitBtn.addEventListener("click", async function() {
        ...
        // Create hidden iframe for form submission (NO REDIRECT)
        ...
        addField(FORM_ENTRY_IDS.orderId, currentOrderId);
        addField(FORM_ENTRY_IDS.orderDate, new Date().toLocaleString());
        addField(FORM_ENTRY_IDS.fullName, window.deliveryDetails?.fullName || currentCustomer?.fullName || "");
        addField(FORM_ENTRY_IDS.email, window.deliveryDetails?.email || currentCustomer?.email || "");
        addField(FORM_ENTRY_IDS.phone, window.deliveryDetails?.phone || currentCustomer?.phone || "");
        addField(FORM_ENTRY_IDS.address, window.deliveryDetails?.address || currentCustomer?.address || "");
        addField(FORM_ENTRY_IDS.city, window.deliveryDetails?.city || currentCustomer?.city || "");
        addField(FORM_ENTRY_IDS.state, window.deliveryDetails?.state || currentCustomer?.state || "");
        addField(FORM_ENTRY_IDS.pincode, window.deliveryDetails?.pincode || currentCustomer?.pincode || "");
        addField(FORM_ENTRY_IDS.product, currentOrderName);
        addField(FORM_ENTRY_IDS.amount, currentOrderPrice);
        ...
        addField(FORM_ENTRY_IDS.orderType, orderTypeValue);
        
        // For file uploads, we need to use the actual file objects
        // Convert base64 to file
        const paymentFile = dataURLToFile(paymentScreenshotBase64, "payment.jpg");
        const paymentInput = document.createElement('input');
        paymentInput.type = 'file';
        paymentInput.name = FORM_ENTRY_IDS.paymentScreenshot;
        paymentInput.files = [paymentFile];
        ...
        if(currentReferenceImage) {
            ...
            refInput.type = 'file';
            refInput.name = FORM_ENTRY_IDS.referenceImage;
            refInput.files = [referenceFile];
        ...
        // Remove iframe and form after submission (give it time to submit)
        setTimeout(() => {
            document.body.removeChild(iframe);
            document.body.removeChild(form);
        }, 3000);
        
        alert(`✅ ORDER CONFIRMED AND SUBMITTED TO GOOGLE FORM!\n\n📦 Order ID: ${currentOrderId}\n💰 Amount: ₹${currentOrderPrice}\n\n📧 Order data saved in Google Form responses.\n📁 Order saved in Admin Panel (Ctrl+Shift+A)`);
