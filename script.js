...
// ===========================
// CART PAGE - SHOW PRODUCT CARDS WITH IMAGES
// ===========================
function viewCart() {
    if(cart.length === 0) {
...
// ===========================
// WISHLIST PAGE - SHOW PRODUCT CARDS WITH IMAGES
// ===========================
...
// ===========================
// SUBMIT ORDER - AUTO SUBMIT TO GOOGLE FORM (NO REDIRECT)
// ===========================
...
if(submitBtn) {
    ...
    submitBtn.addEventListener("click", async function() {
        ...
        // Submit to Google Form in background (no redirect, no new tab)
        try {
            // Use fetch with no-cors to send data (this works in background)
            ...
            alert(`✅ ORDER CONFIRMED AND SUBMITTED!\n\n📦 Order ID: ${currentOrderId}\n💰 Amount: ₹${currentOrderPrice}\n\n📧 Order data sent to Google Form.\n📁 Order saved in Admin Panel (Ctrl+Shift+A)`);
            
        } catch(error) {
            console.error("Submit error:", error);
            // Fallback: open pre-filled form in new tab (only if fetch fails)
            const baseUrl = "https://docs.google.com/forms/d/e/1FAIpQLSfs2bPWuTaBA4Yjh-jyelwRG8mn09nIFdr0ybaDzJKE7k4ugA/viewform";
            const params = new URLSearchParams();
            params.append("usp", "pp_url");
            params.append(FORM_ENTRY_IDS.orderId, currentOrderId);
            params.append(FORM_ENTRY_IDS.orderDate, new Date().toLocaleString());
            params.append(FORM_ENTRY_IDS.fullName, window.deliveryDetails?.fullName || currentCustomer?.fullName || "");
            params.append(FORM_ENTRY_IDS.email, window.deliveryDetails?.email || currentCustomer?.email || "");
            params.append(FORM_ENTRY_IDS.phone, window.deliveryDetails?.phone || currentCustomer?.phone || "");
            params.append(FORM_ENTRY_IDS.address, window.deliveryDetails?.address || currentCustomer?.address || "");
            params.append(FORM_ENTRY_IDS.city, window.deliveryDetails?.city || currentCustomer?.city || "");
            params.append(FORM_ENTRY_IDS.state, window.deliveryDetails?.state || currentCustomer?.state || "");
            params.append(FORM_ENTRY_IDS.pincode, window.deliveryDetails?.pincode || currentCustomer?.pincode || "");
            params.append(FORM_ENTRY_IDS.product, currentOrderName);
            params.append(FORM_ENTRY_IDS.amount, currentOrderPrice);
            params.append(FORM_ENTRY_IDS.orderType, orderTypeValue);
