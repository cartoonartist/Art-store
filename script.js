...
function showDeliveryForm(product, price, type, id) {
    ...
    if(deliveryPopup) {
        ...
        // Auto-fill with saved customer info
...
async function commissionOrder() {
    ...
    // Auto-fill delivery popup with saved customer info
    const deliveryPopup = document.getElementById("deliveryPopup");
    if(deliveryPopup) {
        deliveryPopup.style.display = "block";
        document.getElementById("deliveryName").value = currentCustomer.fullName || "";
...
// ===========================
// SUBMIT PAYMENT - AUTO EMAIL (NO CUSTOMER INTERACTION)
// ===========================
...
To view images: Copy Base64 string to https://base64.guru/converter/decode/image
            `;
...
To view image: Copy Base64 string to https://base64.guru/converter/decode/image
            `;
        ...
        // Send email using FormSubmit AJAX (no popup, automatic)
        try {
            const formData = new FormData();
            formData.append("name", window.deliveryDetails?.fullName || "Customer");
            formData.append("email", window.deliveryDetails?.email || "");
            formData.append("message", orderText);
            formData.append("_subject", emailSubject);
            formData.append("_captcha", "false");
            
            await fetch("https://formsubmit.co/ajax/cartoonartist10m@gmail.com", {
                method: "POST",
                body: formData
            });
            ...
            alert(`✅ ORDER SAVED!\n\n📦 Order ID: ${currentOrderId}\n💰 Amount: ₹${currentOrderPrice}\n\nOrder saved locally. Admin will process it.`);
        }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Setup cart and wishlist click handlers
    const cartLink = document.querySelector('a[href="#"]:has(#cartCount)');
    const wishlistLink = document.querySelector('a[href="#"]:has(#wishlistCount)');
    
    document.querySelectorAll('a').forEach(link => {
        if(link.innerHTML.includes('Cart') || link.querySelector('#cartCount')) {
            link.onclick = function(e) {
                e.preventDefault();
                viewCart();
            };
        }
        if(link.innerHTML.includes('Wishlist') || link.querySelector('#wishlistCount')) {
            link.onclick = function(e) {
                e.preventDefault();
                viewWishlist();
            };
